...
public class AspectConfiguration {
	@Around("execution(* hocpte.services.*.*(org.springframework.data.domain.Pageable, ..))") // pointcut expression
																								// only apply with these
																								// methods service has
	...
	public Object fixPageIfExceed(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		page = page.getPageSize() > 50 ? PageRequest.of(page.getPageNumber(), 50).withSort(page.getSort()) : page; // set
																													// page
																													// size
																													// to
																													// 50
